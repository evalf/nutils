...
pub struct Offset<M: Map> {
    map: M,
    offset: usize,
}

impl<M: Map> Map for Offset<M> {
    #[inline]
    fn dim_in(&self) -> usize {
        if self.map.dim_out() > 0 {
            self.map.dim_in() + self.offset
        } else {
            0
        }
    }
    ...
    fn apply_inplace(
        ...
        self.map
            .apply_inplace(index, coordinates, stride, offset + self.offset)
    }
    #[inline]
...
#[inline]
fn comp_dim_out_in<M: Map>(map: &M, dim_out: usize, dim_in: usize) -> (usize, usize) {
    let n = map.dim_in().checked_sub(dim_out).unwrap_or(0);
    (dim_out + map.delta_dim() + n, dim_in + n)
}

#[inline]
fn comp_mod_out_in<M: Map>(map: &M, mod_out: usize, mod_in: usize) -> (usize, usize) {
