...
def import_handle_matrix(rowptr, colidx, values, ncols):
    handle = sparse_matrix_t(0)
    ...
    status = f(
        byref(handle),
        SPARSE_INDEX_BASE_ZERO,  # 0-based indices
        c_int(nrows),
        c_int(ncols),
        rowptr[:-1].ctypes,
        rowptr[1:].ctypes,
    ...
    m = HandleMatrix(handle, nrows, ncols, dtype)
...
class MKLMatrix(Matrix):
    ...
    def _create(self):
        return import_handle_matrix(self.rowptr, self.colidx, self.data, self.shape[1])
    ...
    def __add__(self, other):
        if not all(self.shape):
            return self
        m = self._create() + self.convert(other)._create()
    ...
    def T(self):
        if not all(self.shape):
            ...
            rowptr, colidx, values = self._create().transpose().export()
