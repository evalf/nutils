...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::StrideSmallerThanOutputDimension => write!(
                f,
...
pub trait Map {
    ...
    fn apply_inplace(
        ...
        } else if offset + self.dim_out() > stride {
            Err(Error::StrideSmallerThanOutputDimension)
        } else {
            Ok(self.apply_inplace_unchecked(index, coords, stride, offset))
        }
    }
