...
class TestTopologyGlueing( object ):
  ...
  def __init__( self ):
    ...
    for elem in self.topo1: # relabel vertices
      elem.vertices = tuple( element.PrimaryVertex(vertex.id+"/") for vertex in elem.vertices )
    ...
    self.topo = self.topo0.glue( self.topo1, self.coords, tol=1e-4 )
  ...
  def test_2DNodeRelabelingBigMaster( self ):
    ...
    args = self.topo1, self.coords
    numpy.testing.assert_raises( AssertionError, topo0.glue, *args )
  ...
  def test_2DNodeRelabelingBigSlave( self ):
    ...
    args = topo1, self.coords
    numpy.testing.assert_raises( AssertionError, self.topo0.glue, *args )

  def test_StokesBEM( self, visual=False ):
