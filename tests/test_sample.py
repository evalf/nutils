...
class Common:
    ...
    def test_get_evaluable_indices(self):
        assert len(self.desired_indices) == self.desired_nelems
        actual = self.sample.get_evaluable_indices(evaluable.InRange(evaluable.Argument('ielem', (), int), evaluable.constant(self.desired_nelems)))
        for ielem, desired in enumerate(self.desired_indices):
    ...
    def test_get_evaluable_weights(self):
        ...
        actual = self.sample.get_evaluable_weights(evaluable.InRange(evaluable.Argument('ielem', (), int), evaluable.constant(self.desired_nelems)))
    ...
    def test_ones_at(self):
        self.assertEqual(self.sample.bind(function.ones((), int)).eval().tolist(), [1]*self.desired_npoints)

    def test_at_in_integral(self):
        topo, geom = mesh.line(2, space='parent-integral')
        actual = topo.integral(self.sample.bind(function.jacobian(geom)), degree=0)
...
class rectilinear(TestCase):
    ...
    def test_asfunction(self):
        ...
        self.assertTrue(evaluable.iszero(evaluable.asarray(gauss.bind(function.derivative(sampled, arg)))))
