    ...
    def assertFunctionAlmostEqual(self, actual, desired, decimal):
        ...
        if actual.ndim == 2:
            with self.subTest('csr'):
                ...
                mat = matrix.assemble_csr(values, rowptr, colidx, ncols)
                self.assertArrayAlmostEqual(mat.export('dense'), desired, decimal)
...
_check('argsort', evaluable.ArgSort, lambda a: numpy.argsort(a, axis=-1), ANY(3, 9))
_check('unique', evaluable.unique, numpy.unique, numpy.arange(10) % 3)
_check('unique-index', lambda a: evaluable.unique(a, return_index=True)[1], lambda a: numpy.unique(a, return_index=True)[1], numpy.arange(10) % 3)
_check('unique-inverse', lambda a: evaluable.unique(a, return_inverse=True)[1], lambda a: numpy.unique(a, return_inverse=True)[1], numpy.arange(10) % 3)
