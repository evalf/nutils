...
class TestShape(unittest.TestCase):
    ...
    def test_fused(self):
        ...
        topo, geom = mesh.csgmsh(dict(dom=cs.Fused(rect, circ)), elemsize=.1, order=2)
    ...
    def test_revolved2(self):
        ...
        rev = cs.Revolved(line, orientation=cs.Orientation(orig, xaxis=(1,-1)), angle=90)
    ...
    def test_revolved3(self):
        ...
        rev = cs.Revolved(rect, orientation=cs.Orientation(orig, xaxis=(-1,0,1), yaxis=(0,1,0)), angle=-90)
    ...
    def test_revolved_full(self):
        ...
        orig = numpy.array([2., 3., 4.])
        rect = cs.Rectangle(x=cs.Interval(1,2))
        rev = cs.Revolved(rect, orientation=cs.Orientation(orig, xaxis=(-1,0,1), yaxis=(0,1,0)), angle=360)
        shapes = dict(dom=rev,
            left=rev.boundary['side-left'],
            right=rev.boundary['side-right'],
            bottom=rev.boundary['side-bottom'],
    ...
    def test_pipe2(self):
        ...
        pipe = cs.Pipe(line, segments=[[numpy.pi/2,1], [1,0], [2*numpy.pi,-.25]], orientation=cs.Orientation(orig))
    ...
    def test_pipe3(self):
        ...
        pipe = cs.Pipe(rect, segments=[[numpy.pi/2,1,0], [1,0,0], [numpy.pi,0,.5]], orientation=cs.Orientation(orig))
        shapes = dict(dom=pipe,
            front=pipe.boundary['front'],
            back=pipe.boundary['back'],
            left0=pipe.boundary['segment0-left'],
...
class TestAxes2(unittest.TestCase):
    ...
    def test_eye(self):
        numpy.testing.assert_almost_equal(cs._shape.Axes2.eye()[:], numpy.eye(2))

    def test_from_x(self):
        xaxis = numpy.array([-1.,1.])
        axes = cs._shape.Axes2.from_x(xaxis)
        numpy.testing.assert_almost_equal(axes[0], xaxis / numpy.linalg.norm(xaxis))

    def test_rotate(self):
        axes = cs._shape.Axes2.from_x([1,0]).rotate(numpy.pi/2)
