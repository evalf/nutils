...
class Statements:
    '''Statements which can be converted to Python code using `write_py`.'''
...
class If(Statements):
    ...
    def write_py(self, script: TextIOBase, indent: str) -> None:
        if self:
            print(f'{indent}if {self.condition.py_expr}:', file=script)
            self.body.write_py(script, indent+'    ')
...
class ForLoop(Statements):
    ...
    def write_py(self, script: TextIOBase, indent: str) -> None:
        if self:
            print(f'{indent}for {self.var.py_expr} in {self.iterable.py_expr}:', file=script)
            self.body.write_py(script, indent+'    ')
...
class With(Statements):
    ...
    def write_py(self, script: TextIOBase, indent: str) -> None:
        if self:
            if self.as_:
                print(f'{indent}with {self.item.py_expr} as {self.as_.py_expr}:', file=script)
            else:
                print(f'{indent}with {self.item.py_expr}:', file=script)
            if self.body:
                self.body.write_py(script, indent+'    ')
            else:
                print(f'{indent}    pass', file=script)
...
class Exec(Statements):
    ...
    def write_py(self, script: TextIOBase, indent: str) -> None:
        print(f'{indent}{self.expression.py_expr}', file=script)
...
class Assert(Statements):
    ...
    def write_py(self, script: TextIOBase, indent: str) -> None:
        print(f'{indent}assert {self.condition.py_expr}', file=script)
