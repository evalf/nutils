...
def main(nelems: int, etype: str, btype: str, degree: int):
    ...
    # Once all entries of ``u`` are establised, the corresponding solution can
    # be vizualised by sampling values of ``ns.u`` along with physical
    # coordinates ``ns.x``, with the solution vector provided via keyword
    # arguments. The sample members ``tri`` and ``hull`` provide additional
    # inter-point information required for drawing the mesh and element
    # outlines.
    ...
    err = domain.integral('(u - sin(x_0) cosh(x_1))^2 dV' @ ns, degree=degree*2).eval(u=u)**.5
