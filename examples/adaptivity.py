  ...
  with treelog.iter.fraction('level', range(nrefine+1)) as lrange:
    for irefine in lrange:
      ...
      if irefine:
        ...
        indicator = refdom.integral('d(refbasis_n, x)_k d(u, x)_k J(x)' @ ns, degree=degree*2).eval(lhs=lhs)
        indicator -= refdom.boundary.integral('refbasis_n d(u, x)_k n(x)_k J(x)' @ ns, degree=degree*2).eval(lhs=lhs)
      ...
      res = domain.integral('d(basis_n, x)_k d(u, x)_k J(x)' @ ns, degree=degree*2)
      lhs = solver.solve_linear('lhs', res, constrain=cons)

      ndofs = len(ns.basis)
      error = domain.integral('<du^2, sum:k(d(du, x)_k^2)>_i J(x)' @ ns, degree=7).eval(lhs=lhs)**.5
