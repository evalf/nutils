...
def main(size: Length = parse('10cm'),
    ...
    args = dict(φ=numpy.random.normal(0, .5, system.trialshapes['φ'])) # initial condition
    ...
    with log.iter.fraction('timestep', range(round(endtime / timestep))) as steps:
        for istep in steps:
            ...
            args = system.step(timestep=1., timesteptarget='dt', historysuffix='0', arguments=args, tol=1, maxiter=5)
