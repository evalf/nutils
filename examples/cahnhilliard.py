...
def main(nelems:int, etype:str, btype:str, degree:int, epsilon:typing.Optional[float],
  ...
  ns.c = 'cbasis_n ?cdofs_n'
  ns.dc = 'cbasis_n (?cdofs_n - ?cdofs0_n)'
  ns.m = 'mbasis_n ?mdofs_n'
  ...
  lhs = dict(cdofs=numpy.random.normal(0,.5,ns.cbasis.shape), mdofs=numpy.random.normal(0,.5,ns.mbasis.shape)) # initial condition

  with treelog.iter.plain('timestep', itertools.count()) as steps:
   for istep in steps:

    E = sample.eval_integrals(nrg_mix, nrg_iface, nrg_wall, **lhs)
    treelog.user('energy: {0:.3f} ({1[0]:.0f}% mixture, {1[1]:.0f}% interface, {1[2]:.0f}% wall)'.format(sum(E), 100*numpy.array(E)/sum(E)))

    x, c, m = bezier.eval(['x_i', 'c', 'm'] @ ns, **lhs)
    ...
    lhs = solver.optimize(['cdofs', 'mdofs'], nrg, arguments=dict(cdofs0=lhs['cdofs']), lhs0=lhs, tol=1e-10)

  return numpy.hstack([lhs['cdofs'], lhs['mdofs']])
