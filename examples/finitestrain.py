...
def main(nelems: int, etype: str, btype: str, degree: int, poisson: float, angle: float, restol: float, trim: bool):
    ...
    u0 = solver.optimize('u', energy, constrain=cons)
    x, energy = bezier.eval(['x_i', 'energy'] @ ns, u=u0)
    ...
    energy = domain.integral('energy dV' @ ns, degree=degree*2)
    u1 = solver.minimize('u', energy, arguments=dict(u=u0), constrain=cons).solve(restol)
    x, energy = bezier.eval(['x_i', 'energy'] @ ns, u=u1)
