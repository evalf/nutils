...
def main(nelems: 'number of elements' = 12,
  ...
  sqr = domain.boundary.integral('u_k u_k J^:x' @ ns, degree=degree*2)
  wallcons = nutils.solver.optimize('lhs', sqr, droptol=1e-15)

  sqr = domain.boundary['top'].integral('(u_0 - 1)^2 J^:x' @ ns, degree=degree*2)
  ...
  res = domain.integral('(ubasis_ni,j stress_ij + pbasis_n u_k,k) J:x' @ ns, degree=degree*2)
  with nutils.log.context('stokes'):
    lhs0 = nutils.solver.solve_linear('lhs', res, constrain=cons)
    postprocess(domain, ns, lhs=lhs0)

  res += domain.integral('ubasis_ni u_i,j u_j J:x' @ ns, degree=degree*3)
