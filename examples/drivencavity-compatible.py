...
def main(nelems:int, degree:int, reynolds:float):
  ...
  with treelog.context('stokes'):
    dofs0 = solver.solve_linear(['udofs', 'pdofs', 'lm'], [ures, pres, lres])
  ...
  with treelog.context('navierstokes'):
    dofs1 = solver.newton(('udofs', 'pdofs', 'lm'), (ures, pres, lres), lhs0=dofs0).solve(tol=1e-10)
    postprocess(domain, ns, **dofs1)

  return [numpy.hstack([dofs['udofs'].ravel(), dofs['pdofs'], dofs['lm']]) for dofs in [dofs0, dofs1]]
