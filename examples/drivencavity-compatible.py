...
def main(nelems: 'number of elements' = 12,
  ...
  res = domain.integral('(ubasis_ni,j stress_ij + pbasis_n (u_k,k + l) + lbasis_n p) J:x' @ ns, degree=2*degree)
  res += domain.boundary.integral('(N ubasis_ni - (ubasis_ni,j + ubasis_nj,i) n:x_j) (u_i - uwall_i) J^:x / Re' @ ns, degree=2*degree)
  with nutils.log.context('stokes'):
    lhs0 = nutils.solver.solve_linear('lhs', res)
    postprocess(domain, ns, lhs=lhs0)

  res += domain.integral('ubasis_ni u_i,j u_j J:x' @ ns, degree=3*degree)
...
def postprocess(domain, ns, every=.05, spacing=.01, **arguments):
  ...
  div = domain.integral('(u_k,k)^2 J:x' @ ns, degree=1).eval(**arguments)**.5
  ...
  sqr = domain.integral('((u_0 - stream_,1)^2 + (u_1 + stream_,0)^2) J:x' @ ns, degree=4)
