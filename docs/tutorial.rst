...
:mod:`nutils.expression_v2` for the complete documentation.
...
with an index, e.g.  ``2 c_i`` and ``c_i A_jk``.  Repeated indices are summed,
e.g. ``A_ii`` is the trace of ``d`` and ``A_ij c_j`` is the matrix-vector
